...
def mocked_boto3():
    ...
    def patcher(client_return_value=None, paginate_return_value=None):
        ...
        if client_return_value is not None:
            mock_boto3.client.return_value = client_return_value
        else:
            mock_boto3.client.get_paginator.return_value = paginator
            if paginate_return_value is not None:
                paginator.paginate.return_value = paginate_return_value
            return mock_boto3
...
def mocked_tempfile():
    ...
    def patcher(mkdtemp_return_value=None):
        ...
        mocked_tempfile.mkdtemp.return_value = mkdtemp_return_value
...
def test_close_should_request_rmtree_from_shutil_if_tmp_dir_path_exists(mocked_boto3, mocked_shutil, mocked_tempfile,
                                                                        mocked_urlparse):
    mocked_tempfile(mkdtemp_return_value='/tmp')
...
def test_close_should_not_request_rmtree_from_shutil_if_tmp_dir_path_does_not_exist(mocked_boto3, mocked_shutil,
    ...
    mocked_tempfile(mkdtemp_return_value=None)
    mocked_urlparse((None, 'bucket', 'path/'))
    s3_walker = S3Walker(fs_url)
...
def test_init_should_set_client_to_boto3_client(mocked_boto3, mocked_urlparse):
    mocked_boto3(client_return_value={})
    mocked_urlparse((None, 'bucket', 'path/'))
...
#     next(walker._listdir('/path/'), None)
#
#     mock_boto3.client.get_paginator.paginate.assert_called_with(Bucket='bucket', Prefix='path/', Delimiter='/')
#
#
...
#
#     mock_boto3.client.get_paginator.paginate.assert_called_with(Bucket='bucket', Prefix='', Delimiter='/')
#
#
